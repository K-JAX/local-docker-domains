...
source "$(dirname "$0")/../lib/docker-integration/env-parser.sh"
...
main() {
    ...
    # Get domains for other components
    local domains=($(get_all_domains))
    
    # Pass to other components
    source "$(dirname "$0")/../dependencies/hostctl/wrapper.sh"
    setup_project_domains "$PROJECT_TITLE" "${domains[@]}"
}
